...
# tunables
my $encoding;
my $fonttype = "Open Sans, sans-serif";
my $imagewidth = 1920;          # max width, pixels
...
my $colors = "perl";             # color theme
my $bgcolors = "#ffffff";       # background color theme
my $nameattrfile;               # file holding function attributes
my $timemax;                    # (override the) sum of the counts
my $factor = 1;                 # factor to scale counts by
my $hash = 1;                   # color by function name
...
if ($time2) {
	my $speedup = sprintf "%+.3f", 100*($time1 / $time2 - 1);
	$subtitletext = "Overall speedup: $speedup%" if not $subtitletext;
}
...
while (my ($id, $node) = each %Node) {
	...
	if ($func eq "" and $depth == 0) {
		if ($time2) {
			my $before = sprintf "%.2f", 100 * ($time1 / $time2);
			my $speedup = sprintf "%+.3f", 100*($time1 / $time2 - 1);
			$info = "all ($samples_txt $countname, before $before%, after 100.00%, speedup $speedup%)";
		} else {
			$info = "all ($samples_txt $countname, 100.00%)";
		}
	} else {
		my $pct = sprintf "%.3f", ((100 * $samples) / ($timemax * $factor));
		...
		unless (defined $delta) {
			...
			my $deltapct = sprintf "%.3f", ((100 * $d) / ($timemax * $factor));
			$deltapct = $d > 0 ? "+$deltapct" : $deltapct;
			my $beforepct = sprintf "%.3f", ((100 * ($samples-$delta)) / ($timemax * $factor));
			$info = "$escaped_func (before $beforepct%, after $pct%, change $deltapct%)";
		}
	}
	...
	if ($func eq "--") {
		...
		$color = color_scale($delta / $samples, 1);
	} elsif ($palette) {
