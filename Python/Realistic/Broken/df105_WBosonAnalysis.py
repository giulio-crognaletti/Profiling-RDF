...
time_waste_code = f"int volatile s = 0; for (int i = 0; i < {BREAK_REPS}; ++i){{s += i;}}"
...
for s in samples:
    ...
    df[s] = df[s].Filter(f" {time_waste_code} return trigE || trigM;")\
                 .Filter(f" {time_waste_code} return met_et > 30000;")

    # Find events with exactly one good lepton
    df[s] = df[s].Define("good_lep", f" {time_waste_code} return lep_isTightID && lep_pt > 35000 && lep_ptcone30 / lep_pt < 0.1 && lep_etcone20 / lep_pt < 0.1;")\
                 .Filter(f" {time_waste_code} return ROOT::VecOps::Sum(good_lep) == 1;")

    # Apply additional cuts in case the lepton is an electron or muon
    df[s] = df[s].Define("idx", f"{time_waste_code} return ROOT::VecOps::ArgMax(good_lep);")\
...
for s in samples:
    if "data" in s:
        ...
        df[s] = df[s].Define("weight", "{} return scaleFactor_ELE * scaleFactor_MUON * scaleFactor_LepTRIGGER * scaleFactor_PILEUP * mcWeight * {} / {} * {};".format(time_waste_code,xsecs[s], sumws[s], lumi))
